...
def f(state_x, dt):
    ...
    if dt <= 10.:
        ...
        vz_new = vz + az * dt
        Cd_new = Cd
        return np.array([x_new, y_new, z_new, vx_new, vy_new, vz_new, Cd_new])
    else:
...
def satellite_UKF(state0,  fx, hx, dt=1.0):
    """state0 :list. e.g. state0=[EARTH_SEMIMAJOR + 400e3, 1, -1, 1, 7700, 1, 2.2] 
    """

    """ =============== Generate sigma points ==============="""
    ### initialise UKF
    sigmas_generator = MerweScaledSigmaPoints(n=7, alpha=0.1, beta = 2., kappa= -4.)  #kappa = -3.
    ukf = UKF(dim_x=7, dim_z=3, fx = fx, hx = hx, dt = dt, points=sigmas_generator) # take f, h from Jai and Vijay
    # print(ukf.Q)
